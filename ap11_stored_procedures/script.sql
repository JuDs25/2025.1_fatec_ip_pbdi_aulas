...
CREATE TABLE tb_item_pedido(
  ...
  CONSTRAINT fk_pedido FOREIGN KEY(cod_pedido) REFERENCES tb_pedido(cod_pedido)); 1,7
1,7
INSERT INTO tb_item
...
CREATE OR REPLACE PROCEDURE sp_calcula_media (
  VARIADIC p_valores INT[] DEFAULT ARRAY[]::INT[]
)
LANGUAGE plpgsql AS $$
DECLARE
  ...
  v_count INT;
BEGIN
  v_count := array_length(p_valores, 1);
  
  IF v_count IS NULL OR v_count = 0 THEN
    RAISE NOTICE 'Nenhum valor informado. Não é possível calcular a média.';
    RETURN;
  END IF;
  ...
  RAISE NOTICE 'A média é %', v_media / v_count;
END;
$$;
